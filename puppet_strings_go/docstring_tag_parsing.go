...
func (ds *Docstring) defaultTagClosingChars() []rune {
	return[]rune{'>', '}', ']', '"'}
}
...
func (ds *Docstring) parseTagWithOptions(tagName string, text string) (tag *OptionsDocstringTag, err error) {
...
func (ds *Docstring) parseTagWithTypesNameAndDefault(tagName string, text string) (tag *DefaultDocstringTag, err error) {
	name, types, remainText, err := ds.extractTypesAndNameFromText(text, ds.defaultTagOpeningChars(), ds.defaultTagClosingChars())
	if err != nil { return nil, err }
	if name == "" {
		name, remainText, err = ds.extractNameFromText(text)
		if err != nil { return nil, err }
	...
	if strings.HasPrefix(remainText, "(") {
		...
		return &DefaultDocstringTag{
			DocstringTag: DocstringTag{
				TagName: tagName,
				Text: before + remainText,
				Types: types,
				Name: name,
			},
			Defaults: defaults,
